...
generate_certs() {
    ...
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout "$BUILD_DIR/certs/avs.key" -out "$BUILD_DIR/certs/avs.crt" -subj "/CN=avs.aerospike.svc"
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout "$BUILD_DIR/certs/avs-ingress.key" -out "$BUILD_DIR/certs/avs-ingress.crt" -subj "/CN=avs-ingress.aerospike.svc"
}
